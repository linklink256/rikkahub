...
private fun buildWorkspacePrompt(workspace: WorkspaceEntity, cwd: String? = null): String = buildString {
    ...
    appendLine("- The subagents directory is mounted at `/agents`. Each subagent role is a subdirectory `/agents/<role-name>/` containing an `AGENT.md` (with `name`, `description`, optional `group`, `tools`, `model`, `reasoningLevel`/`reasoning` frontmatter, followed by the role's system prompt body). Use the native workspace tools to manage subagent definitions: read `/agents/<role-name>/AGENT.md` to view a role, write/edit it to create or update a role, and delete the directory to remove a role (e.g. `rm -rf /agents/<role-name>` via `workspace_shell`). Do NOT invent special subagent management tools; the workspace file tools are the canonical way.")
